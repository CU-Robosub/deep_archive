...
class WaypointNavigator(object):
        ...
        if(self.waypoint is not None):
            ...
            if self.movement_mode == YAW_MODE:
                ...
                if(abs(math.degrees(dyaw)) < 7.5):
                    drive_f64 = Float64()
                    drive_f64.data = self.currentDrive + min(xy_dist, 1.5)
    ...
    def run(self):
        ...
        self.depth_pub = rospy.Publisher('/local_control/pid/depth/setpoint', Float64, queue_size=10)
        self.drive_pub = rospy.Publisher('/local_control/pid/drive/setpoint', Float64, queue_size=10)
        self.strafe_pub = rospy.Publisher('/local_control/pid/strafe/setpoint', Float64, queue_size=10)
        self.yaw_pub = rospy.Publisher('/local_control/pid/yaw/setpoint', Float64, queue_size=10)

        # get current accumulated strafe and drive to use for adjustments
        self.drive_state_sub = rospy.Subscriber("/local_control/pid/drive/state", Float64, self.driveStateCallback)
        self.strafe_state_sub = rospy.Subscriber("/local_control/pid/strafe/state", Float64, self.strafeStateCallback)
