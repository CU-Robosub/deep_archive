...
class Search(Objective):
    ...
    def _loadSearchAlg(self, searchAlg):
        ...
        if searchAlg.lower() == 'simple':
            rospy.loginfo("Using Simple Search Algorithm")
            self.search = SimpleSearch()
        elif searchAlg.lower() == 'zigzag':
            rospy.loginfo("Using ZigZag Search Algorithm")
            self.search = ZigZagSearch()
        elif searchAlg.lower() == 'halfhalf':
            rospy.loginfo("Using HalfHalf Search Algorithm")
...
class SimpleSearch(SearchAlg):
    ...
    #     pass
    def get_path(self, cur_pose, target_pose):
        rospy.loginfo("CURRENT POSE: {}".format(cur_pose))
        rospy.loginfo("TARGET POSE: {}".format(target_pose))
        return [ [ target_pose, self.wait_time ], [None, self.wait_time] ]
#        return [ [ target_pose, self.wait_time ], [cur_pose, self.wait_time] ]
